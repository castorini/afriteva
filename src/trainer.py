...
def write_train_metric(train_metrics, train_time, step, writer_type: str, logger: logging.Logger, eval_steps=None, summary_writer=None, wandb_run=None):
    ...
    elif writer_type == "wandb" and wandb_run is not None:
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        logging.info(
            f"Loading dataset from train: {data_args.train_file} and evaluation: {data_args.validation_file}")
        data_files = {}
    ...
    if model_args.tokenizer_name:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_args.tokenizer_name, cache_dir=model_args.cache_dir, use_fast=model_args.use_fast_tokenizer, config=config
        )
    ...
    if model_args.config_name:
        config = T5Config.from_pretrained(
            model_args.config_name, cache_dir=model_args.cache_dir, vocab_size=len(
                tokenizer)
        )
    elif model_args.model_name_or_path:
        config = T5Config.from_pretrained(
    ...
    if training_args.adafactor:
        ...
        logging.info("Using the Adafactor optimizer")
        optimizer = optax.adafactor(
            learning_rate=linear_decay_lr_schedule_fn,
        )
    else:
        logging.info("Using the AdamW optimizer")
