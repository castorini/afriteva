...
from dataset import AfriT5Dataset
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output_path", type=str, required=True, help="Name of model prefix",
    )
    parser.add_argument(
...
def main():
    ...
    dataset = AfriT5Dataset(source=params.input)
    dataloader = DataLoader(dataset, shuffle=True, batch_size=1000)
    tokenizer = SentencePieceUnigramTokenizer()
    # Customized training
    tokenizer.train_from_iterator(dataloader, vocab_size=params.vocab_size)

    # Save files to disk
    if not os.path.exists(params.output_path):
        os.mkdir(params.output_path)
    tokenizer.save(params.output_path + "/tokenizer.json")
